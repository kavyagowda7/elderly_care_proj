...
public class ElderAppointmentsActivity extends AppCompatActivity {
        ...
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot s : snapshot.getChildren()) {
                    ...
                    if (ap == null || ap.status == null) continue;

                    // ✅ AUTO-COMPLETE ONLY AFTER DAY ENDS (CONFIRMED ONLY)
                    if (ap.status.equalsIgnoreCase("Confirmed")
                            && ap.date.compareTo(todayStr) < 0) {
                ...
                // ✅ SORT: Pending → Confirmed → Completed
    ...
    private boolean isPastAppointment(AppointmentModel m) {
        return m.createdAt < System.currentTimeMillis();
    }
