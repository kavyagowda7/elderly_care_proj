...
public class DoctorAdapter extends RecyclerView.Adapter<DoctorAdapter.Holder> {
    ...
    public void onBindViewHolder(@NonNull Holder h, int pos) {
        ...
        // AVAILABILITY BASED ON WORKING DAYS
        ...
        if (d.workingDays != null && today != null) {
            Boolean v = d.workingDays.get(today);
            isAvailableToday = v != null && v;
        }

        if (isAvailableToday) {
            h.tvAvailability.setText("Available Today");
            h.tvAvailability.setTextColor(0xFF2E7D32); // Green
            h.btnBook.setEnabled(true);
        } else {
            h.tvAvailability.setText("Not Available Today");
            h.tvAvailability.setTextColor(0xFFD32F2F); // Red
            h.btnBook.setEnabled(false);
        }
