...
public class FullScreenAlarmActivity extends AppCompatActivity {
    ...
    String alarmId, medicineId, userId;
    MediaPlayer mp;
    ...
    private void loadAlarmDetails() {
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override public void onDataChange(@NonNull DataSnapshot ds) {
                ...
                String mName = ds.child("medicineName").getValue(String.class);
                if (mName != null) txtMedicine.setText("Time to take " + mName);
            }
    ...
    private void snoozeAlarm(int minutes) {
        if (medicineId == null || userId == null) {
            // still schedule a general snooze broadcast to reopen FullScreenAlarmActivity
            scheduleOneOffSnooze(minutes, "unknown_med", alarmId);
            return;
        }
        String newAlarmId = "snooze_" + System.currentTimeMillis();
        long triggerAt = System.currentTimeMillis() + minutes * 60L * 1000L;

        // Optional: save a small entry in RTDB (helps debugging / visibility)
        Map<String, Object> snoozeObj = new HashMap<>();
        snoozeObj.put("medicineId", medicineId);
        snoozeObj.put("medicineName", txtMedicine.getText().toString());
        snoozeObj.put("time", triggerAt);
        snoozeObj.put("status", "snoozed");
        DatabaseReference snoRef = FirebaseDatabase.getInstance()
                .getReference("alarms")
                .child(userId)
                .child(newAlarmId);
        snoRef.setValue(snoozeObj);

        // schedule the system alarm
        scheduleOneOffSnooze(minutes, medicineId, newAlarmId);
    }
